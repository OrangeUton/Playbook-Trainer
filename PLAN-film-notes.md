...
**The fix: the browser's File System Access API**, granted once (a single one-time permission prompt to pick a folder — e.g. a `film-log/` directory inside this same project), after which the app can read and write real files in it directly, no server, no Electron migration, no repeated prompts per save. Sessions and entries are written as real JSON files, screenshots as real image files (with a small thumbnail alongside for fast list rendering) — the same project directory the plan and research docs already live in, so an agent just reads them like any other file. This removes finding #1 (the bridge gap) entirely instead of working around it with an export button, and removes the localStorage/IndexedDB quota risk at the same time, since it's writing to disk, not a browser storage quota.
